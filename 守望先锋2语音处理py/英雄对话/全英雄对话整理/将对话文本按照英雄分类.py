...
# ==============================
# 1. 功能开关
ENABLE_AUDIO_COPY = False  # True: 复制音频到待上传文件夹; False: 仅生成文本文件
...
AUDIO_ZH_FOLDER = r'G:\守望语音\ow-domina-ver\对话(zh)\HeroConvo'
UPLOAD_FOLDER = r'G:\守望语音\ow-domina-ver\待上传'
...
def collect_audio_files(audio_folder, lang):
    ...
    if not os.path.exists(audio_folder):
        print(f"警告：音频路径不存在 {audio_folder}")
        return audio_map
    ...
    for root, _, files in os.walk(audio_folder):
        for filename in files:
            if filename.endswith('.ogg'):
                ...
                if len(file_parts) >= 3:
                    audio_id = file_parts[2].replace('.ogg', '')
